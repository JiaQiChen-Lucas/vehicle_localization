...
from .metrics import get_cpu_usage, calculate_cpu_usage, get_cpu_temp, get_memory_usage, get_aicore_aicpu_usage
...
# 元数据
DEVICE_TYPE = "OrangePi AIpro"
VEHICLE_INFERENCE_TIME = 13.5   # 推理耗时
PRIORITY = 1        # 优先级
...
metrics = {
    ...
    "aicore_usage": 0,  # AI Core 的使用率
    "aicpu_usage": 0,   # AI CPU 的使用率
}
...
def update_metrics(file_path: str):
    ...
    try:
        ...
        # 调用函数获取 Aicore 和 Aicpu 使用率
        usage = get_aicore_aicpu_usage()

        metrics["aicore_usage"] = usage["aicore_usage"]
        metrics["aicpu_usage"] = usage["aicpu_usage"]

        Logger.success(f"系统指标已更新: CPU={metrics['cpu_usage']:.2f}%, 温度={metrics['cpu_temp']:.2f}°C, 内存={metrics['memory_usage']:.2f}%, Aicore Usage: {metrics['aicore_usage']}, Aicpu Usage: {usage['aicpu_usage']}")
        ...
        log_entry = {
            ...
            "aicore_usage": metrics["aicore_usage"],
            "aicpu_usage": metrics["aicpu_usage"]
        }
    ...
    try:
        client.add_naming_instance(
            ...
            metadata={
                ...
                "aicore_usage": "{0:.2f}".format(metrics["aicore_usage"]),
                "aicpu_usage": "{0:.2f}".format(metrics["aicpu_usage"]),
