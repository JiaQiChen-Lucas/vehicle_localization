...
CANN_MODEL = "./model/best_Pose_RepVGG_ReLU_train_opset12_deploy.om"
...
async def imageSeg_yolov8m(image: UploadFile, timeout: float = Form(2.0), service_type: str = Form("circle")):
    TARGET_NODE_URL = ""
    service = None

    if service_type == "circle":
        service = yolov8m_seg_choose_service_strategy_by_circle()
        if service is None:
            raise HTTPException(status_code=503, detail="没有可用的推理服务")
        else:
            TARGET_NODE_URL = f"http://{service.host}:{service.port}/imageSeg/yolov8m"
    ...
    try:
        ...
        if TARGET_NODE_URL == "":
            raise HTTPException(status_code=503, detail="目标服务不可用")
...
async def imageSeg_yolov8s(image: UploadFile, timeout: float = Form(2.0), service_type: str = Form("circle")):
    TARGET_NODE_URL = ""
    service = None
    ...
    if service_type == "circle":
        ...
        if service is None:
            ...
            TARGET_NODE_URL = f"http://{service.host}:{service.port}/imageSeg/yolov8s"
    elif service_type == "weight":
...
async def queueLength():
    ...
    response_data = {
        ...
        "yolov8s_seg_queue_length": 0,
        "yolov8m_seg_queue_length": 0
    }
...
if __name__ == '__main__':
    ...
    metrics_prefix = os.path.join(sava_prefix, "metrics")
    response_time_prefix = os.path.join(sava_prefix, "response_time")
    image_prefix = os.path.join(sava_prefix, "image")
    ...
    # 创建执行器
    executor = CANNPoolExecutor(cann_model=CANN_MODEL, max_workers=1, cores=1)
