...
from ultralytics.nn.Addmodules import RepVGG_C2f
from ultralytics.nn.Addmodules import Pose_RepVGG
...
def compute_metrics(pred_boxes, gt_boxes, pred_keypoints, gt_keypoints, class_num, pred_boxes_conf, box_iou_threshold=0.5, visibility_threshold=0.5, threshold_ratio=0.1, sigma=0.025):
    ...
    cls_mask = np.full((num_pred, num_gt), fill_value=False)

    for i in range(num_pred):
        for j in range(num_gt):
            iou = calculate_iou(pred_boxes[i], gt_boxes[j])
            iou_matrix[i, j] = iou
            cls_mask[i, j] = (pred_boxes[i][0] == gt_boxes[j][0])

    iou_matrix = iou_matrix * cls_mask

    matches = np.nonzero(iou_matrix >= box_iou_threshold)  # IoU > threshold and classes match
    matches = np.array(matches).T

    # 按照IoU值降序排列
    matches = matches[iou_matrix[matches[:, 0], matches[:, 1]].argsort()[::-1]]

    # 对每个真实框去重，保证每个真实框只匹配一个预测框
    matches = matches[np.unique(matches[:, 1], return_index=True)[1]]

    # 对每个预测框去重，保证每个预测框只匹配一个真实框
    matches = matches[np.unique(matches[:, 0], return_index=True)[1]]

    # 拆分匹配对，将预测框索引存入 new_row_ind，真实框索引存入 new_col_ind
    new_row_ind = matches[:, 0]  # 预测框索引
    new_col_ind = matches[:, 1]  # 真实框索引
    
    for j in new_col_ind:
        cls = gt_boxes[j][0]
...
def validate_model(weight_path, input_image_dir, input_label_dir, conf=0.8, iou_threshold=0.5, visibility_threshold=0.5, threshold_ratio=0.1, sigma=0.025, flag_val=False, verbose=False):
    model = YOLO(weight_path)

    '''
    for name, module in model.named_modules():
        if isinstance(module, RepVGG_C2f) or isinstance(module, Pose_RepVGG):  # 判断模块类型是否是 RepVGG_C2f
    ...
    for image_file in image_files:
        ...
        result = model(image, verbose=verbose, conf=conf)[0]
    ...
    print("模型: ", weight_path)
    # 打印模型信息
    print(model.info())
    print(f"平均耗时(ms): {total_time / len(image_files)}")
    print("加权平均PCK: ", calculate_weighted_pck(dataset_class_correct_keypoints, dataset_class_total_keypoints, dataset_class_total_boxes))
    print("加权平均OKS: ", calculate_weighted_oks(dataset_class_total_oks, dataset_class_total_boxes))
    print("Box-mAP50: ", box_map50)

    if flag_val:
        metrics = model.val(data='./ultralytics/cfg/datasets/engineering_vehicle.yaml',
                  split='test',
                  # iou=0.9,
                  # rect=False,
                  # save_json=True, # if you need to cal coco metrice
                  project='result2/test',
                  name='exp',
                  conf=conf,
                  )
...
if __name__ == '__main__':
    # 输入目录路径
    input_image_dir = '/media/hardDisk1/lucas/code/Wheel-YOLOv8-pose/datasets/vehicle_yolo/images/test'
    input_label_dir = '/media/hardDisk1/lucas/code/Wheel-YOLOv8-pose/datasets/vehicle_yolo/labels/test'
    ...
    weight_path = '/media/hardDisk1/lucas/code/Wheel-YOLOv8-pose/ultralytics-8.2.15/result/Wheel-YOLOv8-pose/ReLU/train/weights/best_pck_oks.pt'

    flag_val = False

    '''
    评估预测框正确性的步骤:
        IoU计算：首先计算预测框和真实框的IoU（Intersection over Union）。
        IoU阈值：确定一个IoU阈值（例如0.5或0.75），常见的选择是0.5。只有当预测框和真实框的IoU大于或等于这个阈值时，才认为它们在位置上是匹配的。
        类别匹配：在位置匹配的基础上，还需要确认预测框和真实框的类别是否一致。如果预测框的类别与真实框的类别不一致，即使IoU满足阈值要求，也不认为是正确的检测。
    '''

    validate_model(weight_path=weight_path, input_image_dir=input_image_dir, input_label_dir=input_label_dir, conf=0.8, iou_threshold=0.5, visibility_threshold=0.5, threshold_ratio=0.025, sigma=0.5, flag_val=flag_val, verbose=True)
