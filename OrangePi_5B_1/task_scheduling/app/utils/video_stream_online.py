...
class VideoStreamSimulator:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    self.isStarted = True
                    Logger.success(f"已启动视频流 {self.stream_id}（远程服务）")
                else:
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    self.isDone = True
                    Logger.success(f"已停止视频流 {self.stream_id}（远程服务）")
                else:
                    Logger.error(f"停止视频流失败：{result.get('message', '未知错误')}")
            else:
    ...
    def get_frame(self) -> Optional[np.ndarray]:
        ...
        get_url = f"http://{REMOTE_HOST}/getFrame"

        params = {"stream_id": self.stream_id}

        try:
            response = requests.get(get_url, params=params, timeout=1)
            if response.status_code == 200:
                # 将返回的字节流转换为 numpy array
                image_bytes = np.frombuffer(response.content, dtype=np.uint8)
                frame = cv2.imdecode(image_bytes, cv2.IMREAD_COLOR)

                # Logger.info(f"获取一帧耗时：{time.time() - start_time}")
                # 追加写入 JSON 行到文件
                log_data = {
                    "timestamp": datetime.now().isoformat(),
                    "get_image_time": time.time() - start_time      # 获取一帧的耗时
                }
                ...
                if frame is not None:
                    Logger.success(f"视频流 {self.stream_id} 成功获取一帧")
                    return frame
                else:
                ...
                # 视频已播放完毕
                Logger.info(f"视频流 {self.stream_id} 已播放完毕")
                self.isDone = True
                return None
            else:
                Logger.warning(f"视频流 {self.stream_id} 获取帧失败，HTTP 状态码：{response.status_code}")
                return None

        except requests.exceptions.RequestException as e:
            Logger.warning(f"视频流 {self.stream_id} 获取帧时发生网络错误：{e}")
            return None
