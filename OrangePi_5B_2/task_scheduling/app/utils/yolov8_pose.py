...
class InferImage:
    def __init__(self, image_shape: Tuple[int, ...], input_data: np.ndarray, padding: Tuple[int, int, int, int], r: float):
...
def image_pre_process(image: np.ndarray) -> InferImage:
    ...
    return InferImage(image.shape, input_data, padding, r)
...
#     def __init__(self, rknn_model: str, max_workers: int = 6, cores: int = 3):
#         """
#         RKNN线程池执行器
#
#         :param rknn_model: RKNN模型文件路径
#         :param func: 推理处理函数 (rknn_instance, input_data) -> result
#         :param max_workers: 最大工作线程数
#         :param cores: NPU核心数
#         """
#         self.rknn_model = rknn_model
#         self.rknn_instances = self._init_rknn_instances(rknn_model, max_workers, cores)
#         self.executor = ThreadPoolExecutor(max_workers=len(self.rknn_instances), thread_name_prefix="RKNNWorker")
#         self.lock = threading.Lock()
#         self.thread_num = 0    # 当前线程编号
#         self.workers = len(self.rknn_instances)     # 总线程数
#         self._active = True
#
...
#             try:
#                 rknn = self.rknn_instances[instance_id % len(self.rknn_instances)]
#                 return detectFrame(rknn=rknn, frame=input_data)
#             except Exception as e:
...
#             try:
#                 rknn.release()
#                 Logger.success(f"RKNN instance {i} released successfully")
#             except Exception as e:
