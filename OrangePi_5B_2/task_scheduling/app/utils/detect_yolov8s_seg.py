...
def yolov8s_seg_choose_service_strategy_by_circle() -> Optional[Service]:
    """
    ...
    Logger.info(f"yolov8s-seg 使用轮询策略选择了服务: {selected.host}:{selected.port}")
    return selected


def yolov8s_seg_choose_service_strategy_by_weight() -> Optional[Service]:
    """根据权重计算得分，选择合适的服务"""
    max_score = float('-inf')
    best_service = None
    ...
    for service in services:
        ...
        inference_time_weight = weight_config.get("yolov8s_seg_inference_time", 0)
        response_time_weight = weight_config.get("yolov8s_seg_response_time", 0)
        ...
        # 推理耗时
        score -= service.yolov8s_seg_inference_time * inference_time_weight
        ...
        Logger.info(f"yolov8s_seg - 服务{service.host}:{service.port} 得分: {score:.2f}")
