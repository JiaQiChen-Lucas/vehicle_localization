...
def track_objects(image_box: np.ndarray, image_cls: np.ndarray, image_score: np.ndarray, image_kpts: np.ndarray,
                  simulator: VideoStreamSimulator) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    进行目标跟踪
    :param image_box: 检测框
    :param image_cls: 类别
    ...
    tracked_results = simulator.bytetracker.update(boxes=image_box, object_classes=image_cls, scores=image_score,
                                                   keypoints=image_kpts)
...
def parse_and_track(concatenateImage: ConcatenateImage, result: dict, simulator: VideoStreamSimulator) -> List[
    ImageResult]:
    """
    ...
    for info in concatenateImage.patch_info:
        ...
        parsed_result.append(ImageResult(image=image, boxes=image_boxes, scores=image_scores, classes=image_classes,
                                         keypoints=image_keypoints, track_ids=image_track_ids))
...
def save_image(parsed_result: List[ImageResult], stream_id: str, KEYPOINTS_THRESH: float = 0.5,
               RESULT_DIR: str = "result/track", IS_LOGGING: bool = False):
    # 创建 result 目录
            ...
            if is_vehicle_static(image_result_queue):
                ...
                while max_detect_count > 0:
                    ...
                    if check_position_is_out(image_result=target_info, stream_id=simulator.stream_id,
                                             RESULT_DIR=IMAGE_SAVE_DIR_PATH, is_save=False,
                    ...
                    Logger.success(
                        f"{simulator.stream_id} 目标已处理完毕，track_id: {target_info.track_ids}, box: {target_info.boxes}, cls: {target_info.classes}, score: {target_info.scores}, keypoints: {target_info.keypoints}")
